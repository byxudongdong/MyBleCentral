...
//        }
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt,
            ...
            //Log.i("写回应","得到写回应");
            if (status == BluetoothGatt.GATT_SUCCESS) {
                broadcastUpdateRead(gatt , WRITE_STATUS, characteristic);
    ...
    private void broadcastUpdateRead(BluetoothGatt gatt,final String action,
                                 final BluetoothGattCharacteristic characteristic) {
        final Intent intent = new Intent(action);
        String mac = gatt.getDevice().getAddress();
        byte[] deviceMac = mac.getBytes();
        intent.putExtra(EXTRA_DATA, deviceMac);
        sendBroadcast(intent);
    }
    ...
    private void broadcastUpdate(BluetoothGatt gatt,final String action,
        ...
        if (data != null && data.length > 0) {
            String mac = gatt.getDevice().getAddress();
            byte[] deviceMac = mac.getBytes();
            byte[] receivedata = new byte[data.length + 17];
            System.arraycopy(deviceMac, 0, receivedata,0, 17);
            System.arraycopy(data, 0, receivedata,17,data.length);
